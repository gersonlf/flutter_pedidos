<?php
    ...
    if (!$mysql) {
        responderErro('Erro executando query em imprimirComanda.php: ' . $mysqli->error);
    }

    if (!$mysql) {
        responderErro('Erro consultando itens de cozinha: ' . $mysqli->error);
    }
    ...
    while ($row = mysqli_fetch_assoc($mysql)) {
        ...
        if (!$mysql2) {
            responderErro('Erro consultando item de cozinha: ' . $mysqli->error);
        }
        ...
        if ($mysql2->num_rows == 0) {
            ...
            if (!$mysql3) {
                responderErro('Erro enviando item para cozinha: ' . $mysqli->error);
            }
    ...
    responderSucesso('Comanda enviada para cozinha', array('itens_enviados' => $itensEnviados));
?>        